...
/**
 * XClass migration job.
 * @since 18.9.0
 * @version $Id$
 */
@Component
@InstantiationStrategy(ComponentInstantiationStrategy.PER_LOOKUP)
@Named(XClassMigratorJob.JOB_TYPE)
@SuppressWarnings("checkstyle:ClassFanOutComplexity")
public class XClassMigratorJob extends AbstractJob<XClassMigratorRequest, JobStatus> implements GroupedJob
{
    static final String JOB_TYPE = "xclassmigration";

    private static final JobGroupPath GROUP = new JobGroupPath(List.of("oldcore", JOB_TYPE));
    ...
    {
        final Collection<?> updatedProperties = request.getUpdatedProperties();
        List<PropertyToUpdate> propertiesToUpdate = new ArrayList<>(updatedProperties.size());
        for (Object p : updatedProperties) {
            if (p instanceof XClassUpdatedEvent.PropertyUpdate(PropertyInterface oldProp, PropertyInterface newProp)) {
        ...
        updateProperties(request.getClassReference(), propertiesToUpdate);
    }
